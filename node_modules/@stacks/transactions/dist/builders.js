...
async function makeUnsignedSTXTokenTransfer(txOptions) {
    ...
    if ('publicKey' in options) {
        ...
        const publicKeys = options.address
            ? sortPublicKeysForAddress(options.publicKeys, options.numSignatures, hashMode, (0, postcondition_types_1.createAddress)(options.address).hash160)
            : options.publicKeys;
...
function mutatingSignAppendMultiSig(transaction, publicKeys, signerKeys, address) {
    ...
    const signer = new signer_1.TransactionSigner(transaction);
    const pubs = address
        ? sortPublicKeysForAddress(publicKeys, transaction.auth.spendingCondition.signaturesRequired, transaction.auth.spendingCondition.hashMode, (0, postcondition_types_1.createAddress)(address).hash160)
        : publicKeys;
