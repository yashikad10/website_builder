...
exports.isClarityAbiPrimitive = (val) => typeof val === 'string';
exports.isClarityAbiBuffer = (val) => val.buffer !== undefined;
exports.isClarityAbiStringAscii = (val) => val['string-ascii'] !== undefined;
exports.isClarityAbiStringUtf8 = (val) => val['string-utf8'] !== undefined;
exports.isClarityAbiResponse = (val) => val.response !== undefined;
exports.isClarityAbiOptional = (val) => val.optional !== undefined;
exports.isClarityAbiTuple = (val) => val.tuple !== undefined;
exports.isClarityAbiList = (val) => val.list !== undefined;
function getTypeUnion(val) {
    if (exports.isClarityAbiPrimitive(val)) {
        if (val === 'uint128') {
...
function encodeClarityValue(input, val) {
    ...
    switch (union.id) {
        case ClarityAbiTypeId.ClarityAbiTypeUInt128:
            return clarity_1.uintCV(val);
        case ClarityAbiTypeId.ClarityAbiTypeInt128:
            return clarity_1.intCV(val);
        case ClarityAbiTypeId.ClarityAbiTypeBool:
            if (val === 'false' || val === '0')
                return clarity_1.falseCV();
            else if (val === 'true' || val === '1')
                return clarity_1.trueCV();
            else
                throw new Error(`Unexpected Clarity bool value: ${JSON.stringify(val)}`);
        case ClarityAbiTypeId.ClarityAbiTypePrincipal:
            if (val.includes('.')) {
                const [addr, name] = val.split('.');
                return clarity_1.contractPrincipalCV(addr, name);
            }
            else {
                return clarity_1.standardPrincipalCV(val);
            }
        case ClarityAbiTypeId.ClarityAbiTypeNone:
            return clarity_1.noneCV();
        case ClarityAbiTypeId.ClarityAbiTypeBuffer:
            return clarity_1.bufferCV(Buffer.from(val, 'utf8'));
        case ClarityAbiTypeId.ClarityAbiTypeStringAscii:
            return stringCV_1.stringAsciiCV(val);
        case ClarityAbiTypeId.ClarityAbiTypeStringUtf8:
            return stringCV_1.stringUtf8CV(val);
    ...
    switch (cv.type) {
        ...
        case clarity_1.ClarityType.Tuple:
            if (union.id == ClarityAbiTypeId.ClarityAbiTypeTuple) {
                const tuple = utils_1.cloneDeep(cv.data);
    ...
    if (filtered.length === 1) {
        ...
        for (let i = 0; i < payload.functionArgs.length; i++) {
            ...
            if (!matchType(payloadArg, abiArg.type)) {
                const argNum = i + 1;
                throw new Error(`Clarity function \`${payload.functionName.content}\` expects argument ${argNum} to be of type ${getTypeString(abiArg.type)}, not ${clarity_1.getCVTypeString(payloadArg)}`);
            }
        }
        return true;
    }
...
function parseToCV(input, type) {
    ...
    if (exports.isClarityAbiPrimitive(type)) {
        if (type === 'uint128') {
            return clarity_1.uintCV(input);
        }
        else if (type === 'int128') {
            return clarity_1.intCV(input);
        }
        else if (type === 'bool') {
            ...
            if (input.includes('.')) {
                ...
                return clarity_1.contractPrincipalCV(address, contractName);
            }
            else {
                return clarity_1.standardPrincipalCV(input);
            }
        }
        else {
    ...
    else if (exports.isClarityAbiBuffer(type)) {
        const inputLength = Buffer.from(input).byteLength;
        if (inputLength > type.buffer.length) {
            throw new Error(`Input exceeds specified buffer length limit of ${type.buffer.length}`);
        }
        return clarity_1.bufferCVFromString(input);
    }
    else if (exports.isClarityAbiResponse(type)) {
        throw new Error(`Contract function contains unsupported Clarity ABI type: ${typeString}`);
    }
    else if (exports.isClarityAbiOptional(type)) {
        throw new Error(`Contract function contains unsupported Clarity ABI type: ${typeString}`);
    }
    else if (exports.isClarityAbiTuple(type)) {
        throw new Error(`Contract function contains unsupported Clarity ABI type: ${typeString}`);
    }
    else if (exports.isClarityAbiList(type)) {
        throw new Error(`Contract function contains unsupported Clarity ABI type: ${typeString}`);
    }
    else {
        throw new Error(`Contract function contains unsupported Clarity ABI type: ${typeString}`);
