...
export const findAndModifyPropsInVDom = (
  ...
  for (let i = 0; i < components.length; i++) {
    if (components[i].id === id) {
      if (obj_key === 'style') {
        components[i].props.style = components[i].props.style || {}; // Ensure style is an object
        components[i].props.style[key] = value;
      } else if (obj_key === 'content') {
        components[i].content = value;
      } else if (obj_key === 'attributes') {
        components[i].props.attributes = components[i].props.attributes || {}; // Ensure attributes is an object
      }
      return;
    }
...
export const handleStyleChange = (
  ...
  setDom(
    produce(dom, (draft) => {
      const element = vDomFind(draft, activeElementId);
      if (element) {
        element.props.style = {
          ...element.props.style,
          [property]: value,
